...
public class Matrix {
	...
	public void setRow(int index, double[] row) {
		elements[index] = Arrays.copyOf(row, row.length);
	}
	...
	public Matrix forwardSubstitution(Matrix goalVector, Matrix L) {
	...
	public Matrix backwardSubstitution(Matrix goalVector, Matrix U) {
		if (goalVector.rows != rows || goalVector.columns != 1) {
	...
	public Matrix inverse() {
		...
		Matrix identity = Matrix.identityMatrix(rows);
		
		Matrix[] LUP = LUPDecomposition(this, false);
		...
		for (int i=0; i<identity.getColumns(); i++) {
			Matrix goalVector = new Matrix(rows, 1);
			goalVector.setColumn(0, identity.getColumn(i));
			Matrix y = LU.forwardSubstitution(P.multiply(goalVector), L);
			
			Matrix x = LU.backwardSubstitution(y, U);
			
			inv.setColumn(i, x.getColumn(0));
		}
