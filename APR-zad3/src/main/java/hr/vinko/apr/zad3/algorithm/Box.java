...
//					simplex.stream().sequential().forEach(x -> System.err.println(f.getValueAt(x)));
//					System.err.println();
					...
					if (stopCondition(xC, simplex, f))
						break;

					double[] xR = reflexion(xC, simplex.get(0));

					xR = fixExplicit(xR);
					xR = fixImplicit(xR, xC);
					xR = fixWorst(xR, simplex.get(1), xC, f);
	...
	private double[] fixWorst(double[] xR, double[] xH2, double[] xC, IFunction f) {
	...
	private double[] fixImplicit(double[] xR, double[] xC) {
		boolean constSatisfied = false;

		while (!constSatisfied) {
			constSatisfied = true;
			for (ImplicitConstraint cons : constraints) {
				if (!cons.check(xR)) {
					xR = moveTowardCentroid(xR, xC);
					constSatisfied = false;
					break;
	...
	private double[] fixExplicit(double[] xR) {
