...
import hr.vinko.apr.zad2.GoldenSection;
...
public class NewtonRaphson implements IAlgorithm {
	...
	public double[] solve(double[] x0, IFunction f) {
		...
		while (true) {
			...
			Matrix L = LUP[0].toLowerMatrix();
			Matrix U = LUP[0].toUpperMatrix();
			

			double[] dx = LUP[0].backwardSubstitution(LUP[0].forwardSubstitution(LUP[1].multiply(DF), L), U).getColumn(0);
			double lambda = 1;
			...
			if (useGoldenSection) {
				...
				hr.vinko.apr.zad2.FunctionWValue f1 = new hr.vinko.apr.zad2.FunctionWValue() {
