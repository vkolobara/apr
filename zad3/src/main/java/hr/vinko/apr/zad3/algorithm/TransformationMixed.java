...
import hr.vinko.apr.zad2.HookeJeves;
import hr.vinko.apr.zad2.Simplex;
...
public class TransformationMixed implements IAlgorithm {
	...
	public double[] solve(double[] x0, IFunction f) {
		...
		while (true) {
			double t = curT;
			hr.vinko.apr.zad2.FunctionWValue u = new hr.vinko.apr.zad2.FunctionWValue() {
	...
	private double[] findInnerPoint(double[] x0) {
		...
		if (!satisfies) {
			...
			hr.vinko.apr.zad2.FunctionWValue G = new hr.vinko.apr.zad2.FunctionWValue() {
				public double getValueAt(double... x) {
					double sum = 0;
